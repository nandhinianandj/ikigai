...
from intersect import Geometry
...
class AnyObjectHandler:
    def legend_artist(self, legend, orig_handle, fontsize, handlebox):
        ...
        patch = patches.Rectangle([x0, y0], width, height, facecolor='red',
                                   edgecolor='black', hatch='xx', lw=3,
                                   transform=handlebox.get_transform())
        handlebox.add_artist(patch)
        return patch

# Register legend handler map for any object
...
def draw_circle(fig, ax, x, y, r, fillcolor):
    c = patches.CirclePolygon(
        xy=(x, y),
        radius=r,
        color=fillcolor)
    ax.add_patch(c)
...
def draw_text(fig, ax, x, y, text, color=[0, 0, 0, 1], fontsize=14, ha="left", va="bottom"):
...
def get_str_labels(data_strs):
    """
    Groups the given list of labels and returns as follows:
        @encodings: {'group_label/id': [''],
                    }

        @rev_encodings: { 'label1': ['group_label/id', ''],
                          'label2': ['group_label/id', ''],
                        }

    """
    encodings = defaultdict(dict)
    rev_encodings = defaultdict(set)
    N = len(data_strs)
    set_collections = {}
    for n, each in enumerate(data_strs):
        for ea in each:
            key = '_'.join([e[0] for e in ea.split(' ')])
            rev_encodings[key].add(n)
            encodings[n][key] = ea

    return encodings, rev_encodings

def get_labels(data, kchoice="autonumber", fill=["number"]):
    """
...
def ikigai_venn4(grouped_labels, names=['A', 'B', 'C', 'D'],
                        rev_encs= {'A': [], 'B': [], 'C':[], 'D':[]},
                                venn_type='ellipse',enc_type='default', **options):
    """
    plots a 4-set Venn diagram

    @type grouped_labels: dict[str, str]
    @type names: list[str]
    @rtype: (Figure, AxesSubplot)

    input
      grouped_labels: a label dict where keys are identified via binary codes ('0001', '0010', '0100', ...),
              hence a valid set could look like: {'0001': 'text 1', '0010': 'text 2', '0100': 'text 3', ...}.
              unmentioned codes are considered as ''.
      names:  group names
      rev_encs: reverse encodings {'skill': <list of groups(names)>,}
      more:   colors, figsize, dpi, fontsize
    ...
    figsize = options.get('figsize', (18, 18))
    radius = options.get('radius', 0.3)
    dpi = options.get('dpi', 96)
    fontsize = options.get('fontsize', 14)
    ...
    if venn_type=='ellipse':

        v_centers.append((0.350, 0.400))
        v_centers.append((0.450, 0.500))
        v_centers.append((0.544, 0.500))
        v_centers.append((0.644, 0.400))
        # body
        ...
        # body
        v_centers.append((0.350, 0.400))
        v_centers.append((0.350, 0.700))
        v_centers.append((0.650, 0.700))
        v_centers.append((0.650, 0.400))
        draw_circle(fig, ax, 0.350, 0.400, radius, colors[0])
        draw_circle(fig, ax, 0.350, 0.700, radius, colors[1])
        draw_circle(fig, ax, 0.650, 0.700, radius, colors[2])
        draw_circle(fig, ax, 0.650, 0.400, radius, colors[3])

    if enc_type=='default':
        draw_text(fig, ax, 0.85, 0.42, grouped_labels.get('0001', ''), fontsize=fontsize)
        draw_text(fig, ax, 0.68, 0.72, grouped_labels.get('0010', ''), fontsize=fontsize)
        draw_text(fig, ax, 0.77, 0.59, grouped_labels.get('0011', ''), fontsize=fontsize)
        draw_text(fig, ax, 0.32, 0.72, grouped_labels.get('0100', ''), fontsize=fontsize)
        draw_text(fig, ax, 0.71, 0.30, grouped_labels.get('0101', ''), fontsize=fontsize)
        draw_text(fig, ax, 0.50, 0.66, grouped_labels.get('0110', ''), fontsize=fontsize)
        draw_text(fig, ax, 0.65, 0.50, grouped_labels.get('0111', ''), fontsize=fontsize)
        draw_text(fig, ax, 0.14, 0.42, grouped_labels.get('1000', ''), fontsize=fontsize)
        draw_text(fig, ax, 0.50, 0.17, grouped_labels.get('1001', ''), fontsize=fontsize)
        draw_text(fig, ax, 0.29, 0.30, grouped_labels.get('1010', ''), fontsize=fontsize)
        draw_text(fig, ax, 0.39, 0.24, grouped_labels.get('1011', ''), fontsize=fontsize)
        draw_text(fig, ax, 0.23, 0.59, grouped_labels.get('1100', ''), fontsize=fontsize)
        draw_text(fig, ax, 0.61, 0.24, grouped_labels.get('1101', ''), fontsize=fontsize)
        draw_text(fig, ax, 0.35, 0.50, grouped_labels.get('1110', ''), fontsize=fontsize)
        draw_text(fig, ax, 0.50, 0.38, grouped_labels.get('1111', ''), fontsize=fontsize)
    else:
        i = 0
        prev_grp = None
        occupied_pos = set()
        for label, groups in rev_encs.items():
            groups = set(groups)
            circles = dict()
            for i in groups:
                chosen_cent = list(v_centers[i])
                chosen_cent += [radius]
                circles[i] = tuple(chosen_cent)
            if len(groups) > 1:
                combos = combinations(groups,2)
                geom = Geometry()
                points = list()
                for comb in combos:
                    pt1, pt2 = geom.circle_intersection(circles[comb[0]],circles[comb[1]])
                    if pt1 or pt2:
                        points.append((pt1,pt2))
                print(points)
                if points and all([bool(x) for x in points]):
                    xpos = stats.mean([pt[0] for pts in points for pt in pts])
                    ypos = stats.mean([pt[1] for pts in points for pt in pts])
                    while (xpos, ypos) in occupied_pos:
                        chosen_incr = random.choice(['x', 'y'])
                        if chosen_incr == 'x':
                            xpos += 0.02
                        else:
                            ypos += 0.02
                    print(label, xpos, ypos)
                    draw_text(fig, ax, xpos, ypos, label, fontsize=fontsize)
                    occupied_pos.add((xpos, ypos))
            else:
                ci = groups.pop()
                print(label, ci)
                if ci==0:
                    xpos = circles[ci][0] - 0.05
                    ypos = circles[ci][1] - 0.05
                elif ci==1:
                    xpos = circles[ci][0] - 0.05
                    ypos = circles[ci][1] + 0.05
                elif ci==2:
    ...
    leg = ax.legend(names, loc='lower right', bbox_to_anchor=(1.0, 0.5), fancybox=True)
    leg.get_frame().set_alpha(0.5)
    ...
    legend_maps = dict()
    i = 0
    for val in grouped_labels.values():
        for k, values in val.items():
            if k not in leg_text:
                i+=1
                new_text = k + '------' + '\t' + values
                if i % 2 == 0:
                    leg_text += new_text + '\t \t |'
    ...
    ax.text(0.205, 0.165, leg_text, transform=ax.transAxes, fontsize=14,
